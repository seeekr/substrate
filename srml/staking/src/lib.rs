...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// # <weight> FLAG: aside from the details below, any function reading `ledger` is also heavy given the right conditions to attack.
		/// - Independent of the arguments. Limited but --potentially-- exploitable complexity.
		/// - Contains a limited number of reads.
		/// - Each call (requires the remainder of the bonded balance to be above `minimum_balance()`)
		///   will cause a new entry to be inserted into a vector (`Ledger.unlocking`) kept in storage. The only way to clean
		///   the aforementioned storage item is also user controlled via `withdraw_unbonded()`.
		///# </weight>
		...
		/// - Contains a limited number of reads, yet the size of which could be large based on `ledger`. // TODO: checking existence via ledger seems to be bad.
		/// - Writes are limited to the `origin` account key.
		///# </weight>
		...
		/// - Triggers the phragmen election. TODO: this needs in-depth analysis.
		/// # </weight>
		...
		/// # <weight>
		/// - Dependent on the argument in terms of the size of write.
		/// - Can only be called by root; safe.
		/// # </weight>
